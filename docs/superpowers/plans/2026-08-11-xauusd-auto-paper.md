...
VERIFIED on 2026-08-12 against the live pre-deploy project (REST probes show HTTP 404 `{"code":"PGRST205","message":"Could not find the table 'public.X' in the schema cache"}` for `paper_trading_profiles`/`scan_runs`/`paper_worker_health`, and `PGRST200` for the `signals`→`paper_trades` embedded relationship): the panel's intended `MIGRATION_REQUIRED` state was **unreachable** — `isMissingSchemaError` matched `error.message` for `42P01`/`PGRST205`, but supabase-js returns PostgREST failures as plain `{ code, message, ... }` objects (not `Error` instances) with the code in `.code`, so the predicate never fired and the panel fell into `WORKER_STANDBY`/"Worker health has not been reported yet.". Fixed: `isMissingSchemaError` moved to `src/lib/xauusd-paper-schema-detection.ts` (code-first: `PGRST205`/`PGRST204`/`PGRST200`/`PGRST202`/`42P01`, message fallback) with a new unit test file (5 tests) pinned to the live shapes; dashboard/signals empty states now say the schema is not deployed instead of "enable Auto-Paper above". Confirmed messaging pre-deploy: amber `NOT_DEPLOYED` badge (renamed from `MIGRATION_REQUIRED` — at 9px mono that reads as "migration expired") + "Auto-Paper is not deployed yet — paper signals appear once the worker is running." (jargon-free; the earlier "Schema migration required — run the paper-trading migrations before enabling." read like an expired state) with the toggle blocked. 362 unit tests, tsc, build, test:db (94/94) all green.
...
Step 5 verified on 2026-08-12 via the PGlite harness: `002` RLS (12/12 — owner sees own rows, all canonical writes denied with 42501, other user sees nothing) and `005` archive (8/8 — only the 31-day terminal signal archived at fixed p_now, trade archived too, both event ledgers untouched, re-run no-op). 001/003/004/006/007 remain green (33/7/10/14/10 — 006 covers configure_xauusd_paper_minute_job: fail-closed without any/partial Vault secrets, scheduled:x job id, xauusd-paper-minute row with `* * * * *`, command embedding the worker URL + apikey + x-worker-secret + empty body, idempotent re-run, archive job intact, authenticated denied with 42501; 007 is the scheduler end-to-end: Vault secrets → configure → EXECUTE the stored cron command through the net.http_post stub (now recording into net.http_request) → assert the recorded request satisfies every createWorkerHandler check — POST by construction, x-worker-secret equal to the vault secret, empty {} body — plus re-fire/re-idempotency; the TS acceptance side is proven by xauusd-paper-handler.test.ts). All 9 migrations apply. The harness is now committed: `bun run test:db` (tools/pgtap-run.mjs + vendored pgTAP under tools/pgtap/, devDependency @electric-sql/pglite). NOTE: the cron migration's `CREATE EXTENSION pg_cron/pg_net/supabase_vault` cannot run in PGlite; the harness strips those three statements and stubs `cron.schedule`, `net.http_post(url,headers,body)` and `vault.decrypted_secrets(name,decrypted_secret)` so the PL/pgSQL bodies still validate. The archive job is scheduled by the migration itself; the minute job only exists after the operator runs `configure_xauusd_paper_minute_job()` with all three Vault secrets present. 357 unit tests (20 static contract), tsc, build, focused lint, diff --check all green; full `bun run lint` shows only pre-existing prettier debt (types.ts generated + earlier-task files; `.worktrees/` doubles the count) — the three focused-list files with debt (xauusd-market-data.ts, oanda-xauusd-provider.ts, paper-trade-state.ts) were prettier-cleaned this task.
