...
**State: LIVE (2026-08-12).** All five activation conditions hold on `mggqzhcacqthwoygmrhg`: migration 10 (`20260812000000`, keyless provider CHECK) pushed; worker Edge function deployed; `XAUUSD_WORKER_CRON_SECRET` set; `configure_xauusd_paper_minute_job()` scheduled the minute cron (`xauusd-paper-minute`, `* * * * *`, active alongside `xauusd-paper-archive`); provider health `ok=true` (`TV_OANDA_FEED`); owner profile `enabled=true` (0.01 lot XAUUSD, activated 2026-08-12 18:31 UTC). The worker posts health every minute and the first enabled cycle wrote 6 scan runs. The panel in the live preview reads `WORKER_HEALTHY` (green) with the toggle on. The cron secret was generated by the operator and stored identically in the Edge-function secrets and the Vault (`xauusd_worker_cron_secret`); Vault also holds `project_url` and `publishable_key`. `SUPABASE_SERVICE_ROLE_KEY` is auto-injected by the platform (the CLI refuses `SUPABASE_*` names) — no operator copy needed.
