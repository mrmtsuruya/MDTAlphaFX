...
**State: LIVE AND TRADING (2026-08-13).** All five activation conditions hold on `mggqzhcacqthwoygmrhg`: migration 10 (`20260812000000`, keyless provider CHECK) pushed; worker Edge function deployed; `XAUUSD_WORKER_CRON_SECRET` set; `configure_xauusd_paper_minute_job()` scheduled the minute cron (`xauusd-paper-minute`, `* * * * *`, active alongside `xauusd-paper-archive`); provider health `ok=true` (`TV_OANDA_FEED`); owner profile `enabled=true` (0.01 lot XAUUSD, activated 2026-08-12 18:31 UTC). The worker posts health every minute and the minute cron runs autonomously. **Real paper signals are firing** — as of 2026-08-13 the ledger holds 4 canonical `xauusd_paper_worker` signals (M15 + H1 OPEN with entries/TP1/TP2/SL set, H4 + D1 expired without entry), and the Signal Center's MT5-style position panel shows the open M15 trade with live floating P&L, current price, and the TP2/TP1/CURRENT/ENTRY/SL ladder. The panel reads `WORKER_HEALTHY` (green), toggle on. The cron secret was generated by the operator and stored identically in the Edge-function secrets and the Vault (`xauusd_worker_cron_secret`); Vault also holds `project_url` and `publishable_key`. `SUPABASE_SERVICE_ROLE_KEY` is auto-injected by the platform (the CLI refuses `SUPABASE_*` names) — no operator copy needed.

**Post-activation fixes (2026-08-13):**
- **`candle_gap` on every M5 scan** — GC=F's raw Yahoo timestamps are not epoch-aligned (the live forming bar carries an off-grid stamp like 18:35:15 that advances each fetch, so `latestCompleted` produced a new fingerprint every minute and every entry series ended in a `candle_gap`; D1 failed the same way — its bars anchor 04:00/05:00 UTC shifting across DST plus partial-session bars). Fixed in `src/lib/tv-keyless-provider.ts`: **every** timeframe is now bucketed onto its UTC grid via `bucketCandles` (previously only H4), so M5 forming ticks merge into their candle and D1 lands on whole UTC days. Regression tests pin both shapes (2 new tests, provider suite 10/10). Redeployed the worker.
- **Signal Center history 500 / stuck Loading** — two bugs once real signals existed: (1) `SIGNAL_VIEW_SELECT` embedded bare `market_snapshots(...)`, which PostgREST rejects with PGRST201 (ambiguous: FK via `market_snapshot_id` AND many-to-many via `signal_market_snapshots`); the embed is now pinned to `market_snapshots!signals_market_snapshot_id_fkey(...)`. (2) The design granted authenticated users **no SELECT on `market_snapshots`** (shared content-hash table), so the canonical FK embed resolved to `null` under RLS and `mapPaperSignalListItem` failed closed on "no provider snapshot" → every list call 500'd. Migration `20260813000000` grants authenticated SELECT limited to snapshots referenced by the requesting user's own signals (RLS policy), keeping worker-owned/other-user snapshots invisible.
- Health/degradation drills verified live (2026-08-12): injected `ok=false, provider_unavailable` through the worker's own RPC; the panel showed `WORKER_DEGRADED` + the red code + blocked toggle, and the next real minute cycle recovered it to `WORKER_HEALTHY` without intervention.
...
- Deploy note: the Edge function's `../../src/lib/...` imports resolved to `supabase/src/lib/...` and the first deploy failed with `Module not found`; fixed to `../../../src/lib/...` (repo root) in `supabase/functions/xauusd-paper-worker/index.ts`. The `WARN: failed to read file: open src/integrations/supabase/types` on deploy is the CLI's asset uploader using an extensionless path for an `import type` that is erased at bundle time — harmless, the function deploys and runs.
- `SUPABASE_SERVICE_ROLE_KEY` is auto-injected by the platform (CLI skips `SUPABASE_*` names); the operator's Management API token can fetch it from `/v1/projects/{ref}/api-keys` if ever needed. No broker credentials are needed.
