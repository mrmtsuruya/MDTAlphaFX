  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "System Monitor — MDTAlphaFX" },
      {
        ...
        content: "Local system telemetry — CPU, GPU, memory, disk, network.",
      },
    ],
  }),
  component: System,
...
function bar(
  label: string,
  value: number,
  tone: "long" | "short" | "accent" | "warn",
  extra?: string,
) {
  const color = {
    long: "bg-neon-long",
    short: "bg-neon-short",
    accent: "bg-neon-accent",
    warn: "bg-neon-warn",
  }[tone];
  return (
    ...
    <div className="p-6 space-y-4 animate-fade-up">
      <header>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Local hardware telemetry preview. Browsers can't read CPU/GPU temps directly — this UI
          mocks the layout. A tiny native agent (Python/Rust) posting to a local endpoint or an
          OS-level webhook can populate real values.
        </p>
      </header>
      ...
      <div className="rounded-lg border border-cyber-border bg-cyber-surface p-4">
        <div className="flex items-center gap-2 text-xs font-mono text-muted-foreground">
          <Cpu className="size-3" /> CPU: AMD Ryzen 9 7950X · <Thermometer className="size-3" />{" "}
          58°C · <MemoryStick className="size-3" /> DDR5 32GB · <HardDrive className="size-3" />{" "}
          NVMe 2TB · <Wifi className="size-3" /> 1Gbps
        </div>
      </div>
    </div>
  );
