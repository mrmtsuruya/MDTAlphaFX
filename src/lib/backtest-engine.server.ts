// MT5-style backtest engine (synthetic candle series until a market data provider is wired).
...
export function generateCandles(pair: string, bars: number, seedSalt = 0): Candle[] {
  const rand = rng(hash(pair) + seedSalt);
  const isJPY = pair.endsWith("JPY");
  const isGold = pair === "XAUUSD";
  let price = isGold ? 2600 : isJPY ? 150 : 1.1;
  const vol = (isGold ? 0.006 : 0.0035) / Math.sqrt(24);
  const out: Candle[] = [];
  let drift = 0;
  const now = Date.now();
...
function signalSeries(id: StrategyId, candles: Candle[], p: Record<string, number>): number[] {
  ...
  if (id === "ema_trend") {
    ...
    for (let i = 1; i < n; i++) {
      const h = macd[i] - sline[i], hp = macd[i - 1] - sline[i - 1];
      if (hp <= 0 && h > 0) sig[i] = 1;
    ...
    for (let i = p.period; i < n; i++) {
      ...
      const upper = mid[i] + p.mult * sd, lower = mid[i] - p.mult * sd;
...
export function runBacktest(
  ...
  while (i < candles.length) {
    ...
    let exit = entry, outcome: Trade["outcome"] = "eod", j = i + 1;
    for (; j < Math.min(candles.length, i + 200); j++) {
      const c = candles[j];
      const hitSL = dir === 1 ? c.l <= sl : c.h >= sl;
      const hitTP = dir === 1 ? c.h >= tp : c.l <= tp;
      if (hitSL && hitTP) { exit = sl; outcome = "sl"; break; }
      if (hitSL) { exit = sl; outcome = "sl"; break; }
      if (hitTP) { exit = tp; outcome = "tp"; break; }
    }
  ...
  return { pair, strategy, params, bars, trades, stats: computeStats(trades), equity: equityCurve(trades) };
}
...
function computeStats(trades: Trade[]): BacktestResult["stats"] {
  ...
  for (const v of eq) { peak = Math.max(peak, v); maxDD = Math.max(maxDD, peak - v); }
  ...
  for (const t of trades) {
    if (t.r > 0) { ws++; ls = 0; bw = Math.max(bw, ws); }
    else { ls++; ws = 0; bl = Math.max(bl, ls); }
  }
...
export function monteCarlo(trades: Trade[], sims: number, ruinR: number): MonteCarloResult {
  ...
  const finals: number[] = [], dds: number[] = [], curves: number[][] = [];
  let ruined = 0;
  for (let s = 0; s < sims; s++) {
    let eq = 0, peak = 0, dd = 0, hitRuin = false;
...
const OPT_GRID: Record<StrategyId, Record<string, number[]>> = {
  ...
  rsi_meanrev: { period: [9, 14, 21], oversold: [20, 30], overbought: [70, 80], tpMult: [1.5, 2, 3] },
